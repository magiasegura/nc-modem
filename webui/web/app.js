...
function renderCaps(c) {
  ...
  $('lock-help').textContent = intel
    ...
    : simcom
      ? 'У SIMCom (A7908E-M2) фиксация ставится штатной CLI роутера `interface X mobile lte lock`. Прочитать её обратно нечем, состояние помним сами.'
  ...
  $('at-warn').textContent = intel
    ...
    : simcom
      ? 'CLI-команда роутера меняет состояние модема через QMI. На свой страх и риск.'
