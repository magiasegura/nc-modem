...
Способы фиксации у разных чипсетов несовместимы, поэтому семейство определяется при старте —
по тому, что модем реально умеет, а не по `ATI` (у L860 он отдаёт лишь дату сборки).

| Семейство | Фиксация | Метрики | Соседи |
|---|---|---|---|
...
которые прошивка не поддерживает, в интерфейсе просто не появятся.

### Особенности Intel

- **Фиксация только парой EARFCN + PCI.** Параметра «любая сота на несущей» у `freq_lock` нет,
  поэтому форма фиксации одной несущей для таких модемов скрыта.
- **Номер диапазона выводится из EARFCN** по таблице 3GPP TS 36.101 и кодируется как `100+N` —
  это следует из `AT+XACT?`, где рядом лежат UMTS-диапазоны `1,2,4,5,8` и LTE `101,103,107,120,…`.
- **Текущую фиксацию из модема не прочитать**: `freq_lock` только пишет. Поэтому состояние
